...
class TradeManager:
    ...
    def run():
        ...
        while True:
            ...
            try:
                ...
                # Check BNF and execute trades accordingly. Only for #PPStrategy
                TradeManager.trackBNF()
    ...
    def registerStrategy(strategyInstance):
        ...
        logging.info("Within strategy instance ---", TradeManager.strategyToInstanceMap)
    ...
    def loadAllTradesFromFile():
        ...
        logging.info('TradeManager: Successfully loaded %d trades from json file %s', len(TradeManager.trades),
                     tradesFilepath)
    ...
    def trackSLOrder(trade):
        ...
        if trade.stopLoss == 0:  # Do not place SL order if no stopLoss provided
            return
        if trade.slOrder == None and trade.strategy != "PPStrategy":
            # Place SL order
            TradeManager.placeSLOrder(trade)
        elif trade.slOrder == None and trade.strategy == "PPStrategy":
            TradeManager.placeSLOrder_PPS(trade)
        else:
            if trade.slOrder.orderStatus == OrderStatus.COMPLETE:
                ...
                if trade.slOrder.message is not None and errorString in trade.slOrder.message:
                    message = "SL order {0} for tradeID {1} cancelled by exchange.".format(trade.slOrder.orderId,
                                                                                           trade.tradeID)
    ...
    def placeEmergencyExitOrder(trade):
        ...
        try:
            ...
            logging.error('TradeManager: Failed to place emergency exit order for tradeID %s: Error => %s',
                          trade.tradeID, str(e))
    ...
    def checkAndUpdateTrailSL(trade):
        ...
        updateSL = False
        if newTrailSL > 0 and newTrailSL != trade.stopLoss:
    ...
    def checkAndUpdateMoveToCost(trade):
        ...
        if updateSL == True:
            ...
            try:
                ...
                message = "TradeManager: Move to Cost: Successfully modified stopLoss from {0} to {1} for tradeID {2}".format(
                    counterTrade.stopLoss, omp.newTriggerPrice, counterTrade.tradeID)
                Utils.sendMessageTelegramBot(message)
                logging.info(message)
    ...
    def trackTargetOrder(trade):
        ...
        if trade.targetOrder == None:
            ...
            if trade.targetOrder.orderStatus == OrderStatus.COMPLETE:
                ...
                logging.error(
                    'Target order %s for tradeID %s cancelled outside of Algo. Setting the trade as completed with exit price as current market price.',
    ...
    def placeSLOrder(trade):
        ...
        if oip.direction == Direction.SHORT:
            oip.price = Utils.roundToNSEPrice(trade.entry - trade.entry * trade.slPercentage / 100)
            oip.triggerPrice = Utils.roundToNSEPrice(trade.entry - trade.entry * trade.slPercentage / 100)
        elif oip.direction == Direction.LONG:
            oip.price = Utils.roundToNSEPrice(trade.entry + trade.entry * trade.slPercentage / 100)
            oip.triggerPrice = Utils.roundToNSEPrice(trade.entry + trade.entry * trade.slPercentage / 100)
        oip.qty = trade.qty
        if trade.isFutures == True or trade.isOptions == True:
        ...
        message = "TradeManager: Successfully placed SL order {0} for tradeID {1}".format(trade.slOrder.orderId,
                                                                                          trade.tradeID)
        Utils.sendMessageTelegramBot(message)
        logging.info(message)
        return True
    ...
    def placeTargetOrder(trade, isMarketOrder=False):
        ...
        try:
            ...
            logging.error('TradeManager: Failed to place Target order for tradeID %s: Error => %s', trade.tradeID,
                          str(e))
            return False
        logging.info('TradeManager: Successfully placed Target order %s for tradeID %s', trade.targetOrder.orderId,
                     trade.tradeID)
        return True
    ...
    def cancelEntryOrder(trade):
        ...
        logging.info('TradeManager: Successfully cancelled Entry order %s for tradeID %s', trade.entryOrder.orderId,
                     trade.tradeID)
    ...
    def cancelSLOrder(trade):
        if trade.slOrder == None:
            return
        if trade.slOrder.orderStatus == OrderStatus.CANCELLED or trade.slOrder.orderStatus == OrderStatus.REJECTED:
            return
        try:
        ...
        logging.info('TradeManager: Successfully cancelled SL order %s for tradeID %s', trade.slOrder.orderId,
                     trade.tradeID)
    ...
    def cancelTargetOrder(trade):
        ...
        logging.info('TradeManager: Successfully cancelled Target order %s for tradeID %s', trade.targetOrder.orderId,
                     trade.tradeID)
    ...
    def convertJSONToTrade(jsonData):
        ...
        trade.bnf_target = jsonData['bnf_target']
        trade.bnf_stoploss = jsonData['bnf_stoploss']
        trade.bnf_order_strategy = jsonData['bnf_order_strategy']
        return trade
